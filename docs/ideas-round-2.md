...
|---|---|---|---|
| 1 | **Exposure diff between two instants** — `blastradius diff <version> --from T1 --to T2`: which repos entered exposure, which left, which never changed | The Time Machine proves bitemporal *reads*; a diff is the question an incident commander actually asks next ("what changed since this morning?"). It is the single most obvious missing follow-up and nothing else in the category can answer it | **BUILT** — `blastradius diff`, 19ms, one read epoch |
| 2 | **SARIF 2.1.0 output** from `ci`, uploaded by the workflow to the GitHub Security tab | Turns the gate from "a script that exits 1" into a findings source GitHub renders natively. Unmistakably more than the minimum to qualify | **BUILT** — verified as a live code-scanning alert on the repo (`blast-radius/exposed/npm:debug@4.4.3`, severity high, tool "Blast Radius") |
| 3 | **Minimal global fix set** — the smallest set of dependency changes that clears *every* exposed repo, solved as a set cover over the graph | Remediation today answers per-repo. A platform team wants the one PR that fixes eleven services. Same data, far better answer | **BUILT** — `remediate --minimal`; 6 changes clear all 7 repos where the per-repo plan lists them separately; 5 unit tests |
| 4 | **Query-plan surfacing** — read HydraDB's optimizer warnings and show when a query fell back to a full scan | Nobody else will show the engine's own plan telemetry. It is the deepest possible "we actually used the sponsor tech" signal | partial — **not buildable as specified**: the query response carries no plan or access-path field, and EXPLAIN/PROFILE are silently ignored. Proven and written up as HydraDB finding 11 instead |
| 5 | **API concurrency cap + stats cache with epoch invalidation** | `/api/stats` is a full edge scan across four edge types; parallel dashboard loads saturate the engine. A demo that browns out under a judge clicking fast is a lost demo | **BUILT** — 8 concurrent /api/stats went from ~18s each to 0.83s wall; invalidation verified against an external CLI writer |
| 6 | **Exposure-over-time series** — exposed-repo count at every lockfile capture instant, drawn on the survey timeline | Turns the bitemporal claim into a line you can point at. One image that a static scanner provably cannot produce | **BUILT** — step line on the survey timeline, peak 11 repos, derived from snapshots already fetched |
| 7 | **Click a range ring to filter to that hop count** | The rings are already a real distance scale; making them a control makes the plot an instrument rather than a picture | not built — ran out of time; ranked above the cut but never started |
| 8 | **Graph pan and zoom** with a reset control | Six thousand paths on one canvas needs it. Its absence reads as unfinished the moment a judge tries | **BUILT** — wheel zoom about the pointer, drag pan, reset control |
| 9 | **PR comment formatter** — `ci --format markdown` posting the gate result as a PR comment | Closes the loop on the CI story; the output lands where a developer would actually see it | **BUILT** — `ci --format markdown` |
| 10 | **Print stylesheet** — the dashboard prints as an actual survey sheet | Dead on-world, genuinely useful for an incident write-up, and the kind of detail nobody else will have | not built — ran out of time; ranked above the cut but never started |
| 11 | **`--since` on `ci`** — gate only on exposure that appeared since a given instant | Makes the gate usable on a repo that is already exposed: fail on regressions, not on the backlog | not built — ran out of time; ranked above the cut but never started |
| 12 | **Sticky focus** — click a repo row to lock its chain highlighted, not just hover | Hover state vanishes the moment you move to point at the screen. In a live demo that is the difference between explaining a chain and losing it | **BUILT** — click locks the chain, click again releases |
| 13 | **Exposure badge counts on the sheet index** | The tab bar becomes a status board; a judge sees "5 exposed" before clicking anything | not built — ran out of time; ranked above the cut but never started |
| 14 | **Unknown-version guidance** — a bad version key returns near-matches from the graph instead of an empty report | The single most likely thing a judge types wrong. Recovering gracefully is a production signal | not built — ran out of time; ranked above the cut but never started |
| 15 | **Structured JSON logging** on the API with query ids, elapsed, and consistency | Makes the server legible when something goes wrong on stage | not built — ran out of time; ranked above the cut but never started |
...
| 33 | **Attack-clock replay scrubber** after the run finishes | Lets a judge re-watch a moment | not built — ran out of time; ranked above the cut but never started |
| 34 | **Keyboard shortcuts for every view** (1–7) | Speed | not built — ran out of time; ranked above the cut but never started |
| 35 | **`?` shortcut sheet** overlay | Discoverability | not built — ran out of time; ranked above the cut but never started |
| 36 | **Copy-as-Markdown** on any result table | Gets a finding into Slack in one action | not built — ran out of time; ranked above the cut but never started |
| 37 | **Permalink to a specific exposed repo's chain** | Shareable evidence | not built — ran out of time; ranked above the cut but never started |
| 38 | **Error boundary** per view so one failing panel cannot blank the app | Production signal | not built — ran out of time; ranked above the cut but never started |
| 39 | **Retry-with-backoff banner** when the API is unreachable, with a live countdown | The demo survives a DB restart on stage | not built — ran out of time; ranked above the cut but never started |
| 40 | **Request coalescing** — identical in-flight queries share one engine round trip | Directly fixes what saturated the engine | **BUILT** — folded into #5; identical in-flight reads share one round trip |
| 41 | **Slow-query warning in the UI** when a traversal exceeds a threshold | Honest about latency | not built — ran out of time; ranked above the cut but never started |
...
| 53 | **Cache warming on startup** | Faster first paint | **BUILT** — folded into #5, as an epoch-invalidated cache rather than a warm-up |
...
| 56 | **Explain plan button** in the console | Deep engine | not built — ran out of time; ranked above the cut but never started |
...
| 58 | **Export console results as CSV** | Interop | not built — ran out of time; ranked above the cut but never started |
...
| 70 | **Bulk mark from an advisory** — mark every version an OSV record affects | Realistic workflow | not built — ran out of time; ranked above the cut but never started |
...
| 75 | **Install-script detection** — flag packages with postinstall hooks | Real attack vector | not built — ran out of time; ranked above the cut but never started |
...
| 89 | **Architecture diagram** in the README | Comprehension | not built — ran out of time; ranked above the cut but never started |
| 90 | **Query cookbook** — every Cypher the product runs, documented | Judge-facing proof | not built — ran out of time; ranked above the cut but never started |
...
| 94 | **Fuzz tests on the lockfile parsers** | Robustness | not built — ran out of time; ranked above the cut but never started |
...
| 98 | **Chaos test** — kill the DB mid-traversal | Resilience | not built — ran out of time; ranked above the cut but never started |
...
**Totals: 10 built and verified · 1 partial · 22 ranked but never started (ran out of time) · 67 skipped with a stated reason.**

The 22 "never started" rows were pre-marked BUILT in the first draft of this
file before any of them existed. They are corrected above rather than left
standing: a padded list is worse than a short one.
